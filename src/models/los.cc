...
namespace {
    ...
    int computeAngularPointCount(double radius, double sweep_deg)
    {
        if (radius <= 0.0 || sweep_deg <= 0.0 || ppd <= 0.0)
            return 1;

        const double radius_px = radius * ppd;
        const double circumference_px = radius_px * TWOPI;
        const int points = (int)ceil(circumference_px * (sweep_deg / 360.0));
        return (points < 1 ? 1 : points);
    }

    int normalizeSegmentCount(int requested_segments, int total_points)
    {
        if (requested_segments < 1)
            requested_segments = 1;
        if (total_points < 1)
            total_points = 1;
        if (requested_segments > total_points)
            requested_segments = total_points;
        return requested_segments;
    }

    void buildAngularRadii(std::vector<PropagationRadius> &radii,
                           const site &source,
                           double radius,
                           double altitude,
                           FILE *fd,
                           ...
                           bool los)
    {
        const int total_points = computeAngularPointCount(radius, sweep_deg);
        const int active_segments = normalizeSegmentCount(requested_segments, total_points);
        const int base_points = total_points / active_segments;
        const int extra_points = total_points % active_segments;
        const double segment_sweep_deg = sweep_deg / (double)active_segments;

        radii.clear();
        radii.reserve((size_t)active_segments);

        for (int i = 0; i < active_segments; ++i) {
            PropagationRadius propRadius;
            propRadius.source = source;
            propRadius.radius = radius;
            propRadius.points = base_points + (i < extra_points ? 1 : 0);
            ...
            propRadius.start_angle_rad = (start_deg + (segment_sweep_deg * i)) * DEG2RAD;
            propRadius.stop_angle_rad = (start_deg + (segment_sweep_deg * (i + 1))) * DEG2RAD;
	...
	{
        // LOS is idempotent over the same mask bit; lock-free check is faster.
        return ((GetMask(lat, lon) & 57) == 0);
	}

    bool hasRequiredFresnelClearance(const site &source, const site &destination, int rx_index)
    {
        if (rx_index < 2 || LR.frq_mhz <= 0.0)
            return true;

        constexpr double C_MPS = 299792458.0;
        constexpr double FEET_PER_METER = 3.28083989501312;
        const double lambda_ft = (C_MPS / (LR.frq_mhz * 1e6)) * FEET_PER_METER;
        if (lambda_ft <= 0.0)
            return true;

        const double total_distance_ft = FEET_PER_MILE * path.distance[rx_index];
        if (total_distance_ft <= 0.0)
            return true;

        const double tx_los_alt = source.alt + path.elevation[0];
        const double rx_los_alt = destination.alt + path.elevation[rx_index];
        const double los_delta_alt = rx_los_alt - tx_los_alt;

        for (int i = 1; i < rx_index; ++i) {
            const double d1_ft = FEET_PER_MILE * path.distance[i];
            const double d2_ft = total_distance_ft - d1_ft;

            if (d1_ft <= 0.0 || d2_ft <= 0.0)
                continue;

            const double terrain_ft = (path.elevation[i] == 0.0 ? path.elevation[i] : path.elevation[i] + clutter);
            const double los_alt_ft = tx_los_alt + (los_delta_alt * (d1_ft / total_distance_ft));
            const double fresnel_radius_ft = fzone_clearance * sqrt(lambda_ft * d1_ft * d2_ft / total_distance_ft);
            const double max_allowed_obstruction_ft = los_alt_ft - fresnel_radius_ft;

            if (terrain_ft > max_allowed_obstruction_ft) {
                return false;
            }
        }
    ...
    */
    ...
    void* radiusPropagation(progress_t &progress, void *parameters)
    {
        ...
        progress.total.store((unsigned int)r->points);
        progress.count.store(0);
        if (r->points < 1) {
            if(r->use_threads) {
                free_elev();
        ...
        double rps = (r->stop_angle_rad - r->start_angle_rad) / (double)r->points;
    ...
    /// @brief Wait for all running futures, then clear tracking state.
    void finishProgress()
    {
        for (auto &f : futures) {
            f.get();
...
void PlotLOSPath(struct site source, struct site destination, char mask_value)
{
    ...
    for (x = 0; (bStop == false) && (x < (path.length - 1)) && (path.distance[x] <= max_range); x++) {
        ...
        const bool geometric_los = (cos_horizon_angle >= cos_angle);
        const bool fresnel_clear = geometric_los && hasRequiredFresnelClearance(source, destination, x);

        if (fresnel_clear && ((GetMask(path.lat[x], path.lon[x]) & mask_value) == 0) && (can_process(path.lat[x], path.lon[x]))) {
	...
	for (y = 2; (y < (path.length - 1) && path.distance[y] <= max_range);
		...
		if ( (GetMask(path.lat[y], path.lon[y]) & 248) !=
			...
			azimuth = (Azimuth(source, temp));
				...
				if (fd != NULL && buffer_offset < (int)sizeof(fd_buffer) - 1) {
					int written = snprintf(fd_buffer + buffer_offset,
							       sizeof(fd_buffer) - (size_t)buffer_offset,
							       "%.7f, %.7f, %.3f, %.3f, ",
							       path.lat[y], path.lon[y], azimuth, elevation);
					if (written > 0) {
						int room = (int)sizeof(fd_buffer) - buffer_offset - 1;
						buffer_offset += (written > room ? room : written);
					}
				}
			...
			/* If ERP==0, write path loss to alphanumeric
			   ...
			   or received power level (below), as appropriate. */
				...
				if (LR.erp == 0.0 && fd != NULL && buffer_offset < (int)sizeof(fd_buffer) - 1) {
					int written = snprintf(fd_buffer + buffer_offset,
							       sizeof(fd_buffer) - (size_t)buffer_offset,
							       "%.2f", loss);
					if (written > 0) {
			...
			if (LR.erp != 0.0) {
				...
				if (fd != NULL) {
					if (block && buffer_offset < (int)sizeof(fd_buffer) - 1) {
						int written = snprintf(fd_buffer + buffer_offset,
							       sizeof(fd_buffer) - (size_t)buffer_offset,
							       " *");
						if (written > 0) {
		...
		bool use_threads, uint8_t segments)
{
    ...
    const double los_radius = max_range * KM_PER_MILE;
    const double sweep_deg = (coverage_sector_enabled ? coverage_width_deg : 360.0);
    const double start_deg = normalize_degrees(coverage_azimuth - (sweep_deg / 2.0));
    std::vector<PropagationRadius> radii;
    buildAngularRadii(radii, source, los_radius, altitude, fd, LOS, 0, 0, use_threads,
                      mask_value, segments, start_deg, sweep_deg, true);

    spdlog::debug("LOS angular sweep {:.2f} -> {:.2f} degrees using {} active segments",
                  start_deg, start_deg + sweep_deg, radii.size());

    futures.clear();
    futures.reserve(radii.size());
    thread_progress = std::vector<progress_t>(radii.size());

    for (size_t i = 0; i < radii.size(); i++) {
        thread_progress[i].id = (int)i;
                            ...
                            bool use_threads, uint8_t segments)
{
    ...
    if (segments < 1)
    {
        spdlog::error("Segment number must be >= 1");
        exit(1);
    }
    ...
    std::vector<PropagationRadius> radii;
    buildAngularRadii(radii, source, range, altitude, fd, prop_model, knifeedge, pmenv, use_threads,
                      mask_value, segments, 0.0, 360.0, false);
	spdlog::debug("With {} active segments, our circular area will be divided into {:.2f}-degree slices",
                    radii.size(), 360.0 / (double)radii.size());
